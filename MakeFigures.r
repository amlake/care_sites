...
FilteredCareSites <- read.xlsx("./output/CareSite_VisitDetailCount_FILTERED_122924.xlsx", 1, colNames=T) %>%
  ...
  mutate(across(ends_with("Percent"), ~as.numeric(gsub("%", "", .))/100)) %>%
  mutate(MajorityVisitType = case_when(Outpt_Percent > 0.75 ~ "Outpatient", Inpt_Percent > 0.75 ~ "Inpatient",Emer_Percent > 0.75 ~ "Emergency",TRUE ~ "Mixed"))
...
count_caresites_byspecialty <- ggplot(figure2 %>% distinct(MappedSpecialty, n_care_sites, Group), 
  ...
  scale_fill_manual(values = c("Medical" = "#E64B35B2", "Surgical" = "#3C5488B2", "Other" = "#00A087B2")) +
  guides(color = "none") +
