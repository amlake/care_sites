...
SpecialtyVisits_Multispecialty <- vroom("SpecialtyVisits_Multispecialty_ENTIRESD.txt")
colnames(SpecialtyVisits_Multispecialty) <- c("GRID","JD_CODE","JD_CODE_COUNT","FIRSTDATE","MINAGE","MAXAGE")
...
PhecodeDemo <- SpecialtyVisits_Multispecialty %>% filter(GRID %in% SubjectMaster$GRID)
...
fwrite(CasesAndControls, "SpecialtyVisits_Multi_1orMore.txt.gz", sep="\t", row.names=FALSE, quote=FALSE, append = FALSE)
table(CasesAndControls$phenotype)

for (i in 2:nrow(JD_quantiles_GT10)) {
  fwrite(GetCaseControl(jdcode=JD_quantiles_GT10[i,"JD_CODE"],1), "SpecialtyVisits_Multi_1orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, col.names=FALSE, append = TRUE)
}

####################################
#2 or more cases.
CasesAndControls <- GetCaseControl(jdcode=JD_quantiles_GT10[1,"JD_CODE"],2)
fwrite(CasesAndControls, "SpecialtyVisits_Multi_2orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, append = FALSE)
table(CasesAndControls$phenotype)

for (i in 2:nrow(JD_quantiles_GT10)) {
  fwrite(GetCaseControl(jdcode=JD_quantiles_GT10[i,"JD_CODE"],2), "SpecialtyVisits_Multi_2orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, col.names=FALSE, append = TRUE)
}

#Save the demographics data.
write.table(SubjectMaster, "SpecialtyWAS_Multi_Demographics_MEGA_EUR.txt", sep="\t", row.names=FALSE,quote=FALSE)
write.table(SubjectMaster %>% filter(GRID %in% Cohort_GRIDs_EUR$GRID), "SpecialtyWAS_Multi_Demographics_MEGA_EUR.txt", sep="\t", row.names=FALSE,quote=FALSE)
write.table(SubjectMaster %>% filter(GRID %in% Cohort_GRIDs_AFR$GRID), "SpecialtyWAS_Multi_Demographics_MEGA_AFR.txt", sep="\t", row.names=FALSE,quote=FALSE)

#Identify the gender-specific phenotypes.
GenderCount <- SpecialtyVisits_Multispecialty %>% 
...
PheWASWrapper("SpecialtyVisits_Multi_1orMore.txt.gz") 
PheWASWrapper("SpecialtyVisits_Multi_2orMore.txt.gz") 
