...
SpecialtyVisits_PrimarySpecialty <- vroom("SpecialtyVisits_PrimarySpecialty_ENTIRESD.txt")
colnames(SpecialtyVisits_PrimarySpecialty) <- c("GRID","JD_CODE","JD_CODE_COUNT","FIRSTDATE","MINAGE","MAXAGE")
...
PhecodeDemo <- SpecialtyVisits_PrimarySpecialty %>% filter(GRID %in% SubjectMaster$GRID)
...
fwrite(CasesAndControls, "SpecialtyVisits_PrimaryOnly_1orMore.txt.gz", sep="\t", row.names=FALSE, quote=FALSE, append = FALSE)
table(CasesAndControls$phenotype)

for (i in 2:nrow(JD_quantiles_GT10)) {
  fwrite(GetCaseControl(jdcode=JD_quantiles_GT10[i,"JD_CODE"],1), "SpecialtyVisits_PrimaryOnly_1orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, col.names=FALSE, append = TRUE)
}

####################################
#2 or more cases.
CasesAndControls <- GetCaseControl(jdcode=JD_quantiles_GT10[1,"JD_CODE"],2)
fwrite(CasesAndControls, "SpecialtyVisits_PrimaryOnly_2orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, append = FALSE)
table(CasesAndControls$phenotype)

for (i in 2:nrow(JD_quantiles_GT10)) {
  fwrite(GetCaseControl(jdcode=JD_quantiles_GT10[i,"JD_CODE"],2), "SpecialtyVisits_PrimaryOnly_2orMore.txt.gz", sep="\t", row.names=FALSE,quote=FALSE, col.names=FALSE, append = TRUE)
}

#Save the demographics data.
write.table(SubjectMaster, "SpecialtyWAS_PrimaryOnly_Demographics_MEGA_EUR.txt", sep="\t", row.names=FALSE,quote=FALSE)
write.table(SubjectMaster %>% filter(GRID %in% Cohort_GRIDs_EUR$GRID), "SpecialtyWAS_PrimaryOnly_Demographics_MEGA_EUR.txt", sep="\t", row.names=FALSE,quote=FALSE)
write.table(SubjectMaster %>% filter(GRID %in% Cohort_GRIDs_AFR$GRID), "SpecialtyWAS_PrimaryOnly_Demographics_MEGA_AFR.txt", sep="\t", row.names=FALSE,quote=FALSE)

#Identify the gender-specific phenotypes.
GenderCount <- SpecialtyVisits_PrimarySpecialty %>% 
...
PheWASWrapper("SpecialtyVisits_PrimaryOnly_1orMore.txt.gz") 
PheWASWrapper("SpecialtyVisits_PrimaryOnly_2orMore.txt.gz") 
